...
let
  ...
  collectPlatformSystemModulesNew =
    { features, activeProviders ? [], system, dispatchableArgs ? [], availableContext ? {} }:
    let
      isDarwin = lib.hasSuffix "-darwin" system;
      isLinux = lib.hasSuffix "-linux" system;

      # Filter features/providers whose slot modules require unavailable context
      filterBySlotContext = slot: sources:
        if dispatchableArgs == [] then sources
        else
          let
            filtered = lib.filter (src:
              let requiredArgs = src._requiredContextArgs.${slot} or [];
              in lib.all (name: availableContext ? ${name}) requiredArgs
            ) sources;
            removed = lib.filter (src:
              let requiredArgs = src._requiredContextArgs.${slot} or [];
              in !(lib.all (name: availableContext ? ${name}) requiredArgs)
            ) sources;
            _ = if removed != [] then
      ...
      collectFromSources = sources:
        let
          ...
          platformModules =
            if isLinux then collectTypedModules "linux" (filterBySlotContext "linux" sources)
            else if isDarwin then collectTypedModules "darwin" (filterBySlotContext "darwin" sources)
    ...
    let
      ...
      collectFromSources = sources:
        let
          ...
          platformHome =
            if isLinux then collectTypedModules "homeLinux" (filterBySlotContext "homeLinux" sources)
            else if isDarwin then collectTypedModules "homeDarwin" (filterBySlotContext "homeDarwin" sources)
            else [];
        in
