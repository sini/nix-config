# BGP routing with FRR.
#
# NOTE: This aspect uses NixOS module options (options.services.bgp) for
# composable BGP configuration. Other aspects (bgp-hub, cilium-bgp,
# thunderbolt-mesh-bgp) contribute to services.bgp.* options.
#
# NOTE: The old feature used `settings.bgp.localAsn` — this is not yet
# in den schema. For now, the NixOS option default of 65001 applies.
{ den, lib, ... }:
{
  den.aspects.bgp = {
    settings = {
    ...
    config = den.lib.perHost (
      ...
      let
        ...
        # TODO: Wire den settings for localAsn when schema is ready
        localAsn = host.settings.bgp.localAsn;
      in
      {
        nixos =
          { config, lib, ... }:
