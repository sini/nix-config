# Grafana dashboard with OIDC SSO via KanIDM.
{ den, lib, ... }:
{
  den.aspects.grafana = {
    includes = lib.attrValues den.aspects.grafana._;
    ...
    _ = {
      ...
      secrets = den.lib.perHost (
        { host }:
        {
          secrets = {
            grafana-oidc-secret = {
              rekeyFile = host.environment.secretPath + "/oidc/grafana-oidc-client-secret.age";
              owner = "grafana";
              group = "grafana";
              generator = {
                tags = [ "oidc" ];
            ...
            grafana-secret-key = {
              rekeyFile = host.environment.secretPath + "/grafana-secret-key.age";
              settings.length = "32";
