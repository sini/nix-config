  ...
  den.aspects.services.kanidm = {
    ...
    nixos =
      ...
      let
        ...
        ) (filterAttrs (_: sys: !(sys.public or false)) (mkOAuth2Services env { }));

        oauth2Services = mkOAuth2Services env secretPaths;
      in
      {
        services = {
          kanidm = {
      ...
      let
        ...
        mkOidcSecret = name: {
          "${name}-oidc-client-secret" = {
            rekeyFile = env.secretPath + "/oidc/${name}-oidc-client-secret.age";
            owner = "kanidm";
            group = "kanidm";
            generator = {
              tags = [ "oidc" ];
