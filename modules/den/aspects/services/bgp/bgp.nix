...
}:
{
  ...
  # BGP spoke — marker aspect for hub auto-discovery, includes bgp base
  den.aspects.services.bgp.spoke = {
    includes = [ den.aspects.services.bgp ];
  };

  # BGP hub — uplink role with auto-discovery of spoke hosts and gateway peering
  den.aspects.services.bgp.hub = {
    includes = [ den.aspects.services.bgp ];
    ...
    settings = {
      ...
      defaultOriginateToSpokes = lib.mkOption {
        type = lib.types.bool;
        default = true;
        description = "Whether to originate default route to spoke neighbors";
      };
    };
    ...
    nixos =
      ...
      let
        ...
        # guaranteed by collect-bgp-peers policy)
        spokeNeighbors = map (p: {
          inherit (p) ip asn;
        }) (filter (p: p.hostname != host.name) bgp-peers);

        allNeighbors = gatewayNeighbor ++ spokeNeighbors;

        # Address family config for all neighbors
        addressFamilyNeighbors = listToAttrs (
          map (
            neighbor:
            let
              isGateway = neighbor.ip == gatewayIp;
            in
            {
              name = neighbor.ip;
              value = {
                activate = true;
                nextHopSelf = false;
                defaultOriginate = if isGateway then false else hubSettings.defaultOriginateToSpokes;
              };
            }
          ) allNeighbors
        );
