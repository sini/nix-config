# Headscale VPN coordination server with OIDC SSO via KanIDM.
{ den, lib, ... }:
{
  den.aspects.headscale = {
    includes = lib.attrValues den.aspects.headscale._;
          ...
          nixos =
            { config, ... }:
            {
              age.secrets.headscale-oidc-secret = {
                rekeyFile = host.environment.secretPath + "/oidc/headscale-oidc-client-secret.age";
                generator = {
                  tags = [ "oidc" ];
