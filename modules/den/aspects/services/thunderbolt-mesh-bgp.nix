...
{ den, ... }:
{
  den.aspects.thunderbolt-mesh-bgp = den.lib.perHost (
    { host }:
    let
    ...
    in
    {
      nixos =
        ...
        let
          ...
          # Derives which interface connects to which peer by checking /31 network pairs
          getGatewayForPeer =
          ...
          peerConfigs = lib.filter (peer: peer.gateway != null) (
            map (
              ...
              let
                peerHost = thunderboltPeers.${peerHostname};
                peerMgmtIp = builtins.head peerHost.ipv4;
