# Vault secret management with raft clustering and auto-unseal.
{ den, lib, ... }:
{
  den.aspects.vault = {
    includes = lib.attrValues den.aspects.vault._;
            ...
            let
              ...
              raftPeers = lib.attrNames (
                lib.attrsets.filterAttrs (
                  hostname: _hostConfig: hostname != config.networking.hostName
                ) allVaultHosts
              );
              ...
              mkRaftPeer =
                hostname:
                let
                  certSecret = config.age.secrets."vault-${config.networking.hostName}".path;
                  keySecret = config.age.secrets."vault-${config.networking.hostName}-key".path;
                  caSecret = config.age.secrets.vault-ca.path;
                in
                ''
                  retry_join {
                    leader_tls_servername = "${hostname}"
                    leader_api_addr = "https://${hostname}:8200"
                    leader_ca_cert_file = "${caSecret}"
                    leader_client_cert_file = "${certSecret}"
                    leader_client_key_file = "${keySecret}"
                  }
                '';
            in
            {
        ...
        { host }:
        {
          nixos =
            { config, ... }:
            {
              age.secrets = {
                vault-ca = {
                  rekeyFile = host.environment.secretPath + "/vault/vault-ca.age";
                  owner = "vault";
                  group = "vault";
                };

                "vault-${config.networking.hostName}" = {
                  rekeyFile = host.environment.secretPath + "/vault/vault-${config.networking.hostName}.age";
                  owner = "vault";
                  group = "vault";
                };

                "vault-${config.networking.hostName}-key" = {
                  rekeyFile = host.environment.secretPath + "/vault/vault-${config.networking.hostName}-key.age";
                  owner = "vault";
                  group = "vault";
                  mode = "0400";
                };
                ...
                vault-unseal-key-1 = {
                  rekeyFile = host.environment.secretPath + "/vault/vault-unseal-key-1.age";
                  owner = "vault";
                  group = "vault";
                  mode = "0400";
                };
                ...
                vault-unseal-key-2 = {
                  rekeyFile = host.environment.secretPath + "/vault/vault-unseal-key-2.age";
                  owner = "vault";
                  group = "vault";
                  mode = "0400";
                };
                ...
                vault-unseal-key-3 = {
                  rekeyFile = host.environment.secretPath + "/vault/vault-unseal-key-3.age";
                  owner = "vault";
                  group = "vault";
                  mode = "0400";
                };
