...
const DEFAULT_CONFIG: SecurityConfig = {
  ...
  permissionGate: {
    ...
    patterns: [
      {
        pattern: "rm -rf /",
        description: "Recursive force root delete",
        action: "block",
      },
      {
        pattern: "rm -rf ~",
        description: "Recursive force home delete",
        action: "block",
      },
      {
        pattern: "rm -rf",
      ...
      {
        pattern: "blkdiscard",
        description: "Block device discard",
        action: "block",
      },
    ...
    rules: [
      {
        ...
        onlyIfExists: false,
        blockMessage:
          "Accessing {file} is not allowed — it may contain secrets.",
      },
      {
        id: "ssh-keys",
...
function loadConfig(): SecurityConfig {
  try {
    ...
    return {
      ...
      permissionGate: {
        requireConfirmation:
          parsed.permissionGate?.requireConfirmation ??
          DEFAULT_CONFIG.permissionGate.requireConfirmation,
...
function matchStructural(command: string): StructuralMatch | undefined {
  ...
  if (cmd === "sudo") {
    if (words[1] === "rm")
      return { description: "Privileged delete", pattern: "sudo rm" };
    return { description: "Privileged command", pattern: "sudo" };
  }

  if (cmd === "dd") {
    if (words.some((w) => w.startsWith("of=")))
      return { description: "Disk write operation", pattern: "dd of=" };
    if (words.some((w) => w.startsWith("if=")))
      return { description: "Disk imaging", pattern: "dd if=" };
  }

  if (cmd === "mkfs" || cmd?.startsWith("mkfs."))
    return { description: "Filesystem format", pattern: "mkfs" };
  if (cmd === "shred")
    return { description: "Secure file overwrite", pattern: "shred" };
  if (cmd === "wipefs")
    return { description: "FS signature wipe", pattern: "wipefs" };
  if (cmd === "blkdiscard")
    return { description: "Block device discard", pattern: "blkdiscard" };
  if (cmd === "fdisk" || cmd === "sfdisk" || cmd === "cfdisk")
    return { description: "Disk partitioning", pattern: "fdisk" };
  if (cmd === "parted" || cmd === "sgdisk")
    return { description: "Disk partitioning", pattern: "parted" };

  if (cmd === "chmod") {
    const recursive = hasFlag(words, "r") || hasLongOpt(words, "recursive");
    const hasMode = words.slice(1).some((w) => /^0?777$/.test(w));
    if (recursive && hasMode)
      return {
  ...
  if (cmd === "shutdown")
    return { description: "System shutdown", pattern: "shutdown" };
  if (cmd === "reboot")
    return { description: "System reboot", pattern: "reboot" };

  if (cmd === "docker" || cmd === "podman") {
    const sub = words[1];
    if (sub === "run" || sub === "create") {
      if (words.some((w) => w === "--privileged"))
        return {
          description: "Privileged container",
          pattern: "docker run --privileged",
        };
      if (words.some((w) => w.startsWith("--pid=host")))
        return { description: "Host PID namespace", pattern: "--pid=host" };
      if (words.some((w) => w.startsWith("--network=host")))
        return {
          description: "Host network access",
          pattern: "--network=host",
        };
      if (words.some((w) => w.startsWith("--userns=host")))
        return { description: "Host user namespace", pattern: "--userns=host" };
      if (words.some((w) => w.includes("docker.sock")))
        return { description: "Docker socket mount", pattern: "docker.sock" };
      if (
        words.some((w) => w.startsWith("-v/:") || w.startsWith("--volume=/:"))
      )
        return {
          description: "Root filesystem mount in container",
...
function createConfirmComponent(command: string, description: string) {
  return (
    _tui: {
    ...
    container.addChild(
      new Text(
        theme.fg("dim", "↑/↓ or j/k: scroll • y/enter: allow • n/esc: deny"),
        1,
        0,
      ),
    );
    ...
    function buildWrappedLines(
      contentWidth: number,
    ): Array<{ rendered: string; lineNum: number }> {
    ...
    return {
      render: (width: number) => {
        ...
        const scrollWindow = Math.max(
          0,
          COMMAND_VIEWPORT_LINES - pinned.length,
        );
        const maxScroll = Math.max(0, scrollable.length - scrollWindow);
        scrollOffset = Math.max(0, Math.min(scrollOffset, maxScroll));

        const visibleScroll = scrollable.slice(
          scrollOffset,
          scrollOffset + scrollWindow,
        );
        const visible = [...pinned, ...visibleScroll];
        const linesBelow = Math.max(
          0,
          scrollable.length - (scrollOffset + visibleScroll.length),
        );

        cmdTopBorder.setText(
          scrollOffset > 0
            ? dimBorder(
                `├${"─".repeat(Math.max(0, width - 3))}↑ ${scrollOffset} more`,
              )
        ...
        cmdBottomBorder.setText(
          linesBelow > 0
            ? dimBorder(
                `├${"─".repeat(Math.max(0, width - 3))}↓ ${linesBelow} more`,
              )
            : dimBorder(`└${"─".repeat(Math.max(0, width - 2))}┘`),
        );
...
function setupPermissionGate(pi: ExtensionAPI, config: SecurityConfig): void {
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    if (structural) {
      ...
      matchedRule = config.permissionGate.patterns.find((r) =>
        command.includes(r.pattern),
      );
    }
...
function setupPolicies(pi: ExtensionAPI, config: SecurityConfig): void {
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    for (const rule of rules) {
      ...
      const allowed = (rule.allowedPatterns ?? []).some((p) =>
        matchesFilePattern(absPath, p),
      );
      if (allowed) continue;
      ...
      const msg = (rule.blockMessage ?? "Access denied").replace(
        "{file}",
        targetPath,
      );
