...
function handleClear(tasks: Task[]): ActionResult {
  ...
  return {
    text:
      count > 0
        ? `Plan cleared (${count} tasks removed).`
        : "No plan was active.",
    tasks: [],
  };
}
...
function renderWidgetLines(tasks: Task[], theme: any, width: number): string[] {
  ...
  const bar = tasks
    ...
    .join("");
  lines.push(
    `${theme.bold(theme.fg("accent", "Plan"))} [${bar}] ${theme.fg("muted", `${complete}/${tasks.length}`)}  ${theme.fg("dim", `→${inProgress} ○${pending}`)}`,
  );
...
State persists across conversation branches via tool result details.`,
    ...
    promptSnippet:
      "Track plan progress with init/update/status/review/clear actions",
    promptGuidelines: [
    ...
    renderResult(result: any, { expanded }: any, theme: any, _context: any) {
      ...
      switch (details.action) {
        ...
        case "update": {
          ...
          const taskName =
            idx !== undefined && taskList[idx] ? taskList[idx].name : "?";
