{ den, inputs, ... }:
{
  den.hosts.x86_64-linux.cortex-cuda = {
    ...
    # Delivered as a CHILD host of cortex (see cortex.nix deliveredChildren).
    # The primitive resolves this as a raw entity bypassing the host submodule
    # (gap G6), so the secretPath/public_key submodule defaults never compute.
