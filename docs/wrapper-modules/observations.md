...
1. **Automatically expose eligible features** as `packages.<system>.<name>`
   outputs for `nix run`
1. **Preserve the multi-layer settings resolution** — a wrapped package should
   reflect the same resolved settings a user would get on their host
1. **Allow standalone use** — `nix run .#alacritty` works without any host
   context for Tier 1 features
1. **Allow contextualized use** — `nix run .#git` works with identity injected
   for Tier 2 features
...
1. **Evaluate it** in a minimal context (no full NixOS eval):
   - For Tier 1: just `pkgs` + `inputs`
   - For Tier 2: `pkgs` + `inputs` + a user context object
1. **Read the evaluated home-manager config** to extract:
...
1. **Feed those into a wrapper-module** that produces the derivation
...
1. **How to handle `requires` for wrappers?** Git requires delta, gh, jujutsu,
   lazygit. For a wrapped git package, should those be bundled into the
   wrapper's PATH, or left as separate wrapped packages? Wrapper-modules
   supports `extraPackages` for this.

1. **Stylix/theming integration?** Many of our home modules benefit from stylix
   theme injection. Wrapper modules would need a theming bridge or accept theme
   settings explicitly.

1. **Home-manager persistence?** Wrappers don't have `home.persistence`.
   Features like firefox that rely on persistence directories would only get
   wrapping for the binary config, not the state management. This is acceptable
   — persistence is a deployment concern, not a package concern.

1. **nix-wrapper-modules as a flake input?** Do we vendor it, or add it as an
   input like we do with nvf, firefox-addons, etc.?

## 8. Recommended Next Steps

1. **Add `nix-wrapper-modules` as a flake input**
1. **Implement `mkWrappedPackage` helper** in `modules/flake-parts/features/`
   using Strategy A (dual definition)
1. **Pilot with alacritty and starship** — simplest possible features, validate
   the `nix run` flow
1. **Extend to kitty, bat, eza, zoxide** — build confidence in the pattern
1. **Tackle git** — first Tier 2 feature, validates identity injection
1. **Evaluate Strategy B feasibility** — can we extract HM config automatically
