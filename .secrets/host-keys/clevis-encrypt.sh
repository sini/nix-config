...
# Read passphrase securely and encrypt
read -rs PASSPHRASE
...
# Encrypt - clevis encrypt doesn't need sudo, only decrypt does for TPM2
if ! echo -n "$PASSPHRASE" | sudo clevis encrypt sss "$CLEVIS_CONFIG" -y | sudo tee "$OUTPUT_FILE" >/dev/null; then
